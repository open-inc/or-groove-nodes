...
GrovePiWrapper.prototype.LCDOutput = function(pin, text, color) {
  ...
  bus.writeByteSync(0x62, 0x00, 0)
  bus.writeByteSync(0x62, 0x01, 0)
  bus.writeByteSync(0x62, 0x08, 0xAA)
  bus.writeByteSync(0x62, 0x04, rgbColor[0])
  bus.writeByteSync(0x62, 0x03, rgbColor[1])
  bus.writeByteSync(0x62, 0x02, rgbColor[2])

  // Clear display
  bus.writeByteSync(0x3e, 0x80, 0x01);
  sleep.usleep(50000);

  // Display on, no cursor
  bus.writeByteSync(0x3e, 0x80, 0x08 | 0x04);

  // Two lines
  bus.writeByteSync(0x3e, 0x80, 0x28);
  sleep.usleep(50000);

  // Write first line
  for (var i = 0; i < line1.length; i++) {
    bus.writeByteSync(0x3e, 0x40, line1.charCodeAt(i));
  }

  // Move to second line
  bus.writeByteSync(0x3e, 0x80, 0xC0);

  // Write first line
  for (var i = 0; i < line2.length; i++) {
    bus.writeByteSync(0x3e, 0x40, line2.charCodeAt(i));
  }
